    ...
    try{
        ...
        if(query.data === 'offer info' && state.telegram){
            ...
            await bot.sendPhoto(telegramId, qrCodeBuffer, { caption: `QR-код для подключения по вашей подписке./n/n
                ...
                👥 Приглашено пользователей: <u>${offerInfo.userInviteCount}</u>${offerInfo.nextPayDiscount}</u>/n/n
                `}
                ...
                За двух приглашенных друзей вы получаете <u>бесплатный месяц на любой тариф</u> 🎁
            `.format(), ...state.options});
            return
        }
